...
document.addEventListener("DOMContentLoaded", function () {
	...
	if (copyrightElement) {
		copyrightElement.innerHTML = `©${currentYear} <i class="fa-fw fas fa-star fa-beat" style="color: #ffff00;"></i> By observator9`;
	}
});
...
function showDateTime() {
	...
	const startTime = new Date("2026-08-21T23:33:33");
	const now = new Date();
	...
	if (seconds < oneYearInSeconds) {
		...
		const remainingSecondsAfterDays =
			remainingSecondsAfterYears % (24 * 60 * 60);
		const hours = Math.floor(remainingSecondsAfterDays / (60 * 60));
		const remainingSecondsAfterHours = remainingSecondsAfterDays % (60 * 60);
		const minutes = Math.floor(remainingSecondsAfterHours / 60);
		const sec = remainingSecondsAfterHours % 60;
